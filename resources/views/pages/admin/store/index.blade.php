                    ...
                    <div class="box-body">
                        <table id="manageTable" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                == 'WORKING' ? 'active-column-success' : 'active-column-failure');
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-primary btn-sm {{ $value->status ==
                                                    "WAITING" ? '' : 'hide' }}" onclick="updateFunc('{{ $value->id }}' ,'Xác nhận kích hoạt tài khoản' ,  '{{ route('admin.store.active_store', $value->id) }}')" data-toggle="modal" data-target="#updateModal">Kích hoạt Shop</button>

                                        @if ($textStatusPayment)
                                        @if ($value->payment_history->payment_status !== 'PAID')
                                        <button type="button" class="btn btn-primary btn-sm" onclick="updateFunc('{{ $value->order_id }}' ,'Thanh toán cửa hàng' ,  '{{ route('admin.store.paid_accept', $value->order_id) }}')" data-toggle="modal" data-target="#updateModal">Thanh
                                            toán</button>
                                        @endif
                                        @endif
                                        ...
                                        <button type="button" class="btn btn-danger btn-sm {{ $value->status ==
                                                    "WORKING" ? '' : 'hide' }}" onclick="updateFunc('{{ $value->id }}' ,'Xác nhận ngừng hoạt động tài khoản' ,  '{{ route('admin.store.stop_store', $value->id) }}')" data-toggle="modal" data-target="#updateModal">Ngừng hoạt động</button>


                                        <button type="button" class="btn btn-success btn-sm {{ $value->status ==
                                                    "STOP_WORKING" ? '' : 'hide' }}" onclick="updateFunc('{{ $value->id }}' ,'Xác nhận  hoạt động tài khoản' ,  '{{ route('admin.store.active_for_paid', $value->id) }}')" data-toggle="modal" data-target="#updateModal">Hoạt động</button>


                                        <a type="button" class="btn btn-default" title="Xóa" data-toggle="modal" data-target="#removeModal" onclick="removeFunc('{{ $value->id }}' , '{{ $value->name }}')"><i class="fa fa-trash"></i></a>


                                        <a type="button" class="btn btn-default" title="Chỉnh sửa" href="{{ route('admin.store.edit', $value->store_code) }}"><i class="fa fa-pencil"></i></a>
                                    </td>
