            ...
            <div class="row">
                <div class="col-md-12 col-xs-12">
                  @include('components.admin.popup_error')
                    ...
                    <a class="btn btn-primary" href="{{ route('manage.screen_home.create') }}">Thêm mới {{$title}}</a>
                        ...
                        <div class="box-body">
                            <table id="manageTable" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($listScreenHome as $key => $value)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{ $value->name }}</td>
                                        <td>{{ $value->price }}</td>
                                        ...
                                        <td><a type="button" class="btn btn-default" title="Chỉnh sửa"
                                                href="{{ route('manage.screen_home.edit', $value->id) }}"><i
